...
- `JZViewInflater` 这类强依赖非空 `AttributeSet` 的 inflater，应在 `attrs == null` 时返回 `null`，避免破坏默认创建路径。
...
- Inflaters such as `JZViewInflater` that require a non-null `AttributeSet` should return `null` when `attrs == null`, preserving the default creation path.
