...
class BindingAdapterCodegenTest {
    ...
    @Test
    fun `binding adapter with requireAll false emits call for present attributes`() {
        val partialBindingAdapters = listOf(
            BindingAdapterDescriptor(
                attrs = listOf("app:stateColorRes", "app:stateSizeDp"),
                methodClassName = "com.example.databinding.SampleBindingAdapters",
                methodName = "setViewState",
        ...
        val tree = XmlLayoutParser().parse(xml, "item_partial_binding_adapter")
        val analyzed = LayoutAnalyzerV2(bindingAdapters = partialBindingAdapters).analyze(tree.root)
        val generated = BindingFacadeGeneratorV2(
            packageName = "com.example.generated",
            rPackageName = "com.example",
            bindingAdapters = partialBindingAdapters
        ).generate(
            analyzedRoot = analyzed,
            layoutName = "item_partial_binding_adapter",
            layoutResId = "R.layout.item_partial_binding_adapter",
            useFastPath = true,
            dataBindingVariables = tree.rootMetadata.dataBindingVariables,
            dataBindingImports = tree.rootMetadata.dataBindingImports
        ).toString()
        ...
        assertThat(analyzed.children.single().unsupportedAttributes).isEmpty()
        assertThat(generated).contains(
            "SampleBindingAdapters.setViewState(stateIndicator, com.example.shared.R.color.sample_state)"
        )
    }
